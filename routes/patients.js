...
// Get details for a specific patient (protected)
...
// Alternative add patient endpoint (same as /add-Patient, but named as per spec)
router.post('/add-patient-ext', authMiddleware, (req, res) => {
    try {
        const { name } = req.body;
        const userId = req.user && req.user.userId;
